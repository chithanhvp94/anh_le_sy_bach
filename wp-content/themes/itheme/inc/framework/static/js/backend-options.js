...
var fwBackendOptions = {
	/**
	 * @deprecated Tabs are lazy loaded https://github.com/ThemeFuse/Manager/issues/1174
	 */
			...
			$boxes
				...
				.on('click'+ eventNamespace, '> .hndle, > .handlediv', function(e){
					...
					if ($box.parent().is('.fw-backend-postboxes') && !$box.siblings().length) {
						// Do not close if only one box https://github.com/ThemeFuse/Manager/issues/1094
	...
	(function(){
		var htmlAttrName = 'data-fw-tab-html',
			...
			initAllTabs = function ($el) {
				...
				// fixes https://github.com/ThemeFuse/Manager/issues/1634
				$el.each(function(){
					if ($(this).is(selector)) {
						initTab($(this));
					}
