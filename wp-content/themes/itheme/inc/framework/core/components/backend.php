	...
	private function register_static() {
		 ...
		 */
		{
			...
			wp_register_script(
				...
				fw()->manifest->get_version(),
				false // false fixes https://github.com/ThemeFuse/Manager/issues/1625#issuecomment-224219454
			);
			...
			wp_localize_script( 'fw', '_fw_localized', array(
				...
				'l10n'       => array_merge(
					...
					/**
					 * fixes https://github.com/ThemeFuse/Manager/issues/2381
					 * @since 2.6.14
					 */
		 ...
		 * https://github.com/ThemeFuse/Manager/issues/2198
		 * @deprecated
		 */
		...
		wp_register_script(
			 ...
			 * moment.locale(document.documentElement.lang.slice(0, 2)); // fixes https://github.com/ThemeFuse/Manager/issues/1767
			 */
	...
	public function _filter_admin_footer_text( $html ) {
		if (
			...
			fw_current_screen_match(array(
				'only' => array(
					array('parent_base' => fw()->extensions->manager->get_page_slug()) // Manager Extensions page
				)
			))
		) {
				...
				array(
					fw_html_tag('a', array(
						...
						'href'   => 'https://www.facebook.com/sharer/sharer.php?'. http_build_query(array(
							'u' => 'https://www.facebook.com/akasa.com.vn',
						)),
						'onclick' => 'return !window.open(this.href, \'Facebook\', \'width=640,height=300\')',
					), __('Facebook', 'fw'))
				),
				__('If you like Akasa Media JSC, {wp_review_link}, share on {facebook_share_link}.', 'fw')
			)
			. '</em>';
		} else {
			return $html;
				...
				if (
					...
					if (defined('WP_DEBUG') && WP_DEBUG) {
						FW_Flash_Messages::add(
							...
							.'<p><a href="'. esc_attr('') .'" target="_blank">'. esc_html__('Info about fw-storage', 'fw') .'</a></p>',
							'warning'
						);
					}
