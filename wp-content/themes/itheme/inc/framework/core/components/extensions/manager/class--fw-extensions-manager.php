...
final class _FW_Extensions_Manager
{
			...
			if (is_wp_error($collected)) {
				if (defined('WP_DEBUG') && WP_DEBUG) {
					if ($this->is_extensions_page()) {
						// display this warning only on Manager extensions page
		...
		do {
			...
			if ($_SERVER['REQUEST_METHOD'] === 'POST') {
				...
				if ($supported && $install_result === true) {
					/**
					 * @since 2.6.14
					 * Fixes https://github.com/ThemeFuse/Manager/issues/2330
					 */
		...
		if ($wp_filesystem->exists($destination_wp_fs_dir)) {
			...
			if (!empty($destination_files)) {
				if (
					 ...
					 * Used by https://github.com/ThemeFuse/Manager/issues/1874
					 */
				} else {
	...
	public function _action_enqueue_scripts()
	{
		wp_enqueue_style(
			...
			$this->get_uri('/static/Manager-font-icon/style.css'),
			array(),
	...
	public function _action_admin_notices() {
		...
		/**
		 * In v2.4.12 was done a terrible mistake https://github.com/ThemeFuse/Manager-Extensions-Approval/issues/160
